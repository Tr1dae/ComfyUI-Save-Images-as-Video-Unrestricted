...
class SaveFramesToVideoFFmpeg:
    ...
    def INPUT_TYPES(cls):
           return {
            ...
            "optional": {
                ...
                "output_dir": ("STRING", {"tooltip": "Custom output directory path. If not provided, uses default output directory."}),
            },
    ...
    def save_video(self, images, filename_prefix, fps, codec, pixel_format, output_format,
                   audio=None, audio_codec="aac", audio_bitrate="192k", output_dir=None,
        ...
        if output_dir is not None and output_dir.strip():
            # Custom output directory provided - bypass folder_paths restrictions
            import time

            try:
                # Validate and normalize the custom output directory
                output_dir = os.path.abspath(output_dir.strip())

                # Parse filename_prefix for subfolder and filename
                subfolder = os.path.dirname(os.path.normpath(filename_prefix))
                filename = os.path.basename(os.path.normpath(filename_prefix))

                # Handle variable substitution like folder_paths does
                if "%" in filename_prefix:
                    filename_prefix_processed = filename_prefix.replace("%width%", str(w))
                    filename_prefix_processed = filename_prefix_processed.replace("%height%", str(h))
                    now = time.localtime()
                    filename_prefix_processed = filename_prefix_processed.replace("%year%", str(now.tm_year))
                    filename_prefix_processed = filename_prefix_processed.replace("%month%", str(now.tm_mon).zfill(2))
                    filename_prefix_processed = filename_prefix_processed.replace("%day%", str(now.tm_mday).zfill(2))
                    filename_prefix_processed = filename_prefix_processed.replace("%hour%", str(now.tm_hour).zfill(2))
                    filename_prefix_processed = filename_prefix_processed.replace("%minute%", str(now.tm_min).zfill(2))
                    filename_prefix_processed = filename_prefix_processed.replace("%second%", str(now.tm_sec).zfill(2))

                    subfolder = os.path.dirname(os.path.normpath(filename_prefix_processed))
                    filename = os.path.basename(os.path.normpath(filename_prefix_processed))

                full_output_folder = os.path.join(output_dir, subfolder)

                # Handle counter logic manually
                try:
                    if os.path.exists(full_output_folder):
                        files = os.listdir(full_output_folder)
                        matching_files = []
                        for f in files:
                            if f.startswith(filename + "_") and f.endswith("_"):
                                try:
                                    num_part = f[len(filename) + 1:-1]  # Extract number between filename_ and _
                                    num = int(num_part)
                                    matching_files.append(num)
                                except (ValueError, IndexError):
                                    continue
                        counter = max(matching_files) + 1 if matching_files else 1
                    else:
                        counter = 1
                except (OSError, PermissionError) as e:
                    error_msg = f"Error accessing output directory '{full_output_folder}': {e}"
                ...
                try:
                    ...
                    error_msg = f"Error creating output directory '{full_output_folder}': {e}"
                    log_node_error(self.NODE_LOG_PREFIX, error_msg)
                    return {"ui": {"text": [error_msg]}}

                filename_part_returned = filename
                subfolder_relative_to_output = subfolder

            except Exception as e:
                error_msg = f"Error processing custom output directory '{output_dir}': {e}"
                log_node_error(self.NODE_LOG_PREFIX, error_msg)
                return {"ui": {"text": [error_msg]}}

        else:
            # Use default behavior with folder_paths restrictions
